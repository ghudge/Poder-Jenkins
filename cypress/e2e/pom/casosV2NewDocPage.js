...
class CasosV2Page {
    ...
    titleCV2 = '.text-2xl.text-black.font-bold'
    newCaseB = '#BtnNuevoCaso'
    ...
    clickNewCaseButton() {
        cy.get(this.newCaseB).should('be.visible').click()
    }
    ...
    clickNewCaseDrop() {
        cy.get('#iconBtnBuscarPersonaTR').click()
        cy.get(this.newCaseDrop).should('be.visible').click()
    }
    clickSubjectDrop() {
        cy.get(this.subjectDrop, { timeout: 10000 }).should('be.visible').click()
    }
    clickTribunalDrop() {
        cy.get(this.tribunalDrop, { timeout: 10000 }).should('be.visible').click()
    }
    ...
    clickAsuntos() {
        cy.get(this.asuntos, { timeout: 10000 }).should('be.visible').click()
    }
    clickSiguienteButton() {
        cy.get(this.siguiente).should('be.visible').and('be.enabled').click()
    }
    clickRegistroPartes() {
    ...
    selectDistritoJudicial(data) {
        ...
        // this.clickDropDownOption()
        cy.xpath('//*[@class="dropdown-item active"]/span[2][text()="' + data + '"]').click({ force: true })
        cy.wait(2000)
    }
    selectTribunal(data) {
...
export default new CasosV2Page()